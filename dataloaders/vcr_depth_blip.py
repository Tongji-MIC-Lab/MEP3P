...
class VCR(Dataset):
    ...
    def __getitem__(self, index):
        ...
        features_3d = np.eye(boxes.shape[0], 6)
        crop_depth_img = np.zeros((boxes.shape[0], 64, 64))
        ## box中心x (width)
        features_3d[:,0] = (boxes[:,0] + boxes[:,2]) / 2
        ## box中心y (height)
        features_3d[:,1] = (boxes[:,1] + boxes[:,3]) / 2
        ## 物体宽度
        features_3d[:,3] = boxes[:,2] - boxes[:,0]
        ## 物体高度
        features_3d[:,4] = boxes[:,3] - boxes[:,1]
        ## 物体大小
        features_3d[:,5] = features_3d[:,3]*features_3d[:,4]
        ## box中心z, crop深度图
        for m in range(boxes.shape[0]):
            features_3d[m,2] = depth_img[int(features_3d[m,1]),int(features_3d[m,0])]
            crop_depth_img[m,:,:] = cv2.resize(depth_img[int(boxes[m,1]):int(boxes[m,3]),int(boxes[m,0]):int(boxes[m,2])], (64,64))
        
        max_r = np.max(features_3d, axis=0) # 所有维度最大的值
        # print("1_max_r",max_r)
        min_r = np.min(features_3d, axis=0)
        # print("1_min_r",min_r)
        r_range = max_r - min_r
        features_3d = (features_3d - min_r) / (r_range+1e-9)

        max_r = np.max(crop_depth_img) # 所有维度最大的值
        # print("2_max_r",max_r)
        min_r = np.min(crop_depth_img)
        # print("2_min_r",min_r)
        r_range = max_r - min_r
        crop_depth_img = (crop_depth_img - min_r) / (r_range+1e-9)

        #===================================================================================================
        if self.mode=="answer":
            blip_img_path = os.path.join("../blip2/vcr_img_features_a",item['annot_id'])
            blip_text_path = os.path.join("../blip2/vcr_text_features_a",item['annot_id'])
            blip_mm_path = os.path.join("../blip2/vcr_mm_features_a",item['annot_id'])

            blip_img = np.concatenate([np.load(blip_img_path+"_0.npy"), np.load(blip_img_path+"_1.npy"), np.load(blip_img_path+"_2.npy"), np.load(blip_img_path+"_3.npy")], axis=0)
            blip_mm = np.concatenate([np.load(blip_mm_path+"_0.npy"), np.load(blip_mm_path+"_1.npy"), np.load(blip_mm_path+"_2.npy"), np.load(blip_mm_path+"_3.npy")], axis=0)
        elif self.mode=="rationale":
            blip_img_path = os.path.join("../blip2/vcr_img_features_r",item['annot_id'])
            blip_text_path = os.path.join("../blip2/vcr_text_features_r",item['annot_id'])
            blip_mm_path = os.path.join("../blip2/vcr_mm_features_r",item['annot_id'])
        ...
        instance_dict['features_3d'] = ArrayField(features_3d, padding_value=0)
        instance_dict['crop_depth_img'] = ArrayField(crop_depth_img, padding_value=0)
        instance_dict['boxes'] = ArrayField(boxes, padding_value=-1)
        instance_dict['blip_img'] = ArrayField(blip_img, padding_value=0)
        instance_dict['blip_mm'] = ArrayField(blip_mm, padding_value=0)
